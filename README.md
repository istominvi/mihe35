...
1. **1-я коробка (Поздравление)**
   - локальный файл (по умолчанию): `public/video/congratulation.mp4`
   - внешний URL через переменную окружения: `NEXT_PUBLIC_CONGRATULATION_VIDEO_URL`
2. **3-я коробка (Привет из детства)**
   - встроенное видео VK (по умолчанию): `https://vkvideo.ru/video-236919220_456239017`
   - можно переопределить embed URL через переменную окружения: `NEXT_PUBLIC_CHILDHOOD_VK_EMBED_URL`

Если видео из 1-й коробки не найдено, в модальном окне появится подсказка с тем, что нужно добавить файл или указать переменную окружения.

### Ранее используемый вариант для 3-й коробки

До встраивания VK использовались:
